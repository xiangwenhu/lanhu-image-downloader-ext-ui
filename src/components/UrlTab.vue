<template>
    <n-form ref="refForm" :model="model" :rules="rules" class="form">
        ...
        <n-form-item label="尺寸调整" path="resizeScale">
            <n-input-number v-model:value="model.resizeScale" placeholder="请输入尺寸调整" :max="1" :min="0.5" :step="0.1"
                :disabled="!isValidUrl"></n-input-number>
        </n-form-item>
        ...
        <n-form-item>
            <n-space justify="center" style="width: 100%;">
                <n-popconfirm @positive-click="onToDownload" :disabled="!isValidUrl" negative-text="取消"
                    positive-text="确认">
                    <template #trigger>
...
import { type ConfigParamsInformation, EnumUrlType } from "../types";
import { getDownloadParamsByUrl } from "./utils/index";
import asyncMessenger from "@/utils/messagener";
...
const rules: FormRules = {
    url: [{
        ...
        validator(_rule, value) {
            ...
            if (val.startsWith("https://lanhuapp.com/")) return true;
...
function validateUrl(url: string) {
    const urlInfo = getDownloadParamsByUrl(url, { teamId: '' });
    return urlInfo
}
